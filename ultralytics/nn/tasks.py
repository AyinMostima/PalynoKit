...
# DETECT_CLASS = (Detect_TADDH,Detect,Detect_DyHead,Detect_SEAM, Detect_MultiSEAM, Detect_DyHeadWithDCNV4)
DETECT_CLASS = (Detect,Detect_SEAM, Detect_MultiSEAM,Detect_TADDH)
            ...
            try:
                ...
                if 'Not implemented on the CPU' in str(
                    ...
                    m.stride = torch.tensor([s / x.shape[-2] for x in
                                             _forward(torch.zeros(2, ch, s, s).to(torch.device('cuda')))])  # forward
                else:
                    raise e
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if isinstance(c2, list) and m not in {MutilScaleEdgeInfoGenetator}:
            is_backbone = True
            m_ = m
            m_.backbone = True
        else:
